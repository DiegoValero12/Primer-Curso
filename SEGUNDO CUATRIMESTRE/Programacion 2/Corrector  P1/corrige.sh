...
if test $numero -eq 6; then
  for fichero in $fuentes; do
   if test $fichero == Valores.java || test $fichero == Fruto.java || \
      test $fichero == Planta.java || test $fichero == Coordenada.java || \
      test $fichero == Huerta.java || test $fichero == Persona.java ; then 
...
if test $numfuentes -eq 6; then
  if test $numero -gt 6; then
  ...
  continuar=false
  echo "Error, el fichero fuente necesario es Coordenada.java, Fruto.java, Persona.java, Planta.java, Huerta.java y Valores.java; 0"
fi
